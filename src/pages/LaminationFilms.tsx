...
import { ArrowLeft, Check, Send, Shield, Compass, CheckCircle } from 'lucide-react';
...
export default function LaminationFilms({ onBack, onScrollTo }: LaminationFilmsProps) {
  ...
  // Custom RFQ state
  const [selectedProduct, setSelectedProduct] = useState('BOPP Film (10 Mic)');
  const [rfqWidth, setRfqWidth] = useState('1000');
  const [rfqThickness, setRfqThickness] = useState('10 Mic');
  ...
  const thermalProducts = [
    {
      ...
      image: "/images/products/thermal-gloss-film.png",
    ...
    },
    {
      ...
      image: "/images/products/thermal-matte-film.png",
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 pt-24 pb-16 text-gray-900 grid-overlay">
      ...
      <div className="absolute top-[20%] right-[-10%] w-[500px] h-[500px] bg-brand-orange/5 rounded-full blur-[150px] pointer-events-none" />
      <div className="absolute bottom-[20%] left-[-10%] w-[450px] h-[450px] bg-brand-navy/5 rounded-full blur-[130px] pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            return (
              <button
                ...
                onClick={() => {
                  setActiveTab(tab.id);
                  setSelectedProduct(
                    tab.id === 'bopp' ? boppProducts[0].name :
                    tab.id === 'pvc' ? pvcProducts[0].name :
                    tab.id === 'thermal' ? thermalProducts[0].name :
                    polyesterProducts[0].name
                  );
                }}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 mb-24">
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {product.applications.map((app) => (
                            <span 
                              key={app} 
                              className="inline-flex items-center px-3.5 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-xs font-medium text-gray-700"
                            >
          ...
          {/* Sidebar: Dynamic Category RFQ Desk */}
          <div className="lg:col-span-4">
            <div className="sticky top-28 bg-brand-navy text-white rounded-3xl p-6 sm:p-8 border border-white/10 shadow-2xl relative overflow-hidden text-left">
              ...
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-48 bg-brand-orange/15 rounded-full blur-[50px] pointer-events-none" />

              <div className="relative z-10">
                <span className="text-[10px] font-mono tracking-widest text-brand-orange uppercase font-bold block mb-2">
                  REAL-TIME PORTAL
                </span>
                ...
                <p className="font-sans font-light text-xs text-blue-100/70 leading-relaxed mb-6">
                  Select your film product width, thickness parameter, and estimated volumes. Our technical team responds within 12 business hours.
                </p>
                ...
                <AnimatePresence mode="wait">
                  {isSuccess ? (
                    <motion.div
                      ...
                      key="rfq-success"
                    >
                      <CheckCircle className="w-12 h-12 text-brand-orange mx-auto mb-3 animate-bounce" />
                      <h4 className="font-heading font-bold text-lg text-white mb-2">Parameters Logged</h4>
                      <p className="font-sans font-light text-xs text-blue-100/60 leading-relaxed">
                        Specifications route mapped. Regional dealer dispatch scheduled. Dynamic pricing matrix activated.
                      </p>
                    </motion.div>
                  ) : (
                    <motion.form 
                    ...
                    >
                      ...
                      <div className="flex flex-col">
                        <label className="font-heading font-bold text-[10px] text-blue-200/80 uppercase mb-1.5">Select Target Product *</label>
                        <select
                          value={selectedProduct}
                          onChange={(e) => {
                            setSelectedProduct(e.target.value);
                            // Auto fill specs if matches
                            const allProd = [...boppProducts, ...pvcProducts, ...thermalProducts, ...polyesterProducts];
                            const found = allProd.find(p => p.name === e.target.value);
                            if (found) {
                              if (found.specs.thickness) setRfqThickness(found.specs.thickness);
                            }
                          }}
                          className="px-3.5 py-2.5 rounded-xl bg-white/10 border border-white/15 focus:border-brand-orange text-white text-xs outline-none transition-colors"
                        >
                          <optgroup label="BOPP Solutions" className="bg-brand-navy">
                            {boppProducts.map(p => <option key={p.name} value={p.name}>{p.name}</option>)}
                          </optgroup>
                          <optgroup label="PVC Solutions" className="bg-brand-navy">
                            {pvcProducts.map(p => <option key={p.name} value={p.name}>{p.name}</option>)}
                          </optgroup>
                          <optgroup label="Thermal Solutions" className="bg-brand-navy">
                            {thermalProducts.map(p => <option key={p.name} value={p.name}>{p.name}</option>)}
                          </optgroup>
                          <optgroup label="Polyester & Specialty Solutions" className="bg-brand-navy">
                            {polyesterProducts.map(p => <option key={p.name} value={p.name}>{p.name}</option>)}
                          </optgroup>
                        </select>
                      </div>

                      {/* Width & Thickness parameters inputs */}
                      <div className="grid grid-cols-2 gap-3">
                        <div className="flex flex-col">
                          <label className="font-heading font-bold text-[10px] text-blue-200/80 uppercase mb-1.5">Target Width (mm) *</label>
                          <input
                            type="number"
                            required
                            value={rfqWidth}
                        ...
                        <div className="flex flex-col">
                          <label className="font-heading font-bold text-[10px] text-blue-200/80 uppercase mb-1.5">Thickness (Mic) *</label>
                          <input
                            type="text"
                            required
                            value={rfqThickness}
                            onChange={(e) => setRfqThickness(e.target.value)}
                            placeholder="e.g. 10 Mic"
                            className="px-3.5 py-2.5 rounded-xl bg-white/10 border border-white/15 focus:border-brand-orange text-white text-xs outline-none transition-colors"
                          />
                        </div>
                      </div>
                      ...
                      <button
                        ...
                        className="w-full py-3.5 rounded-xl bg-brand-orange hover:bg-amber-600 text-white font-sans font-extrabold text-xs tracking-wider uppercase transition-colors duration-300 disabled:opacity-50 cursor-pointer flex items-center justify-center space-x-2"
                      >
                        {isSubmitting ? (
                          <>
        ...
        <div className="p-8 rounded-3xl bg-slate-900 text-white flex flex-col md:flex-row items-center justify-between gap-6 border border-white/10 text-left relative overflow-hidden">
          <div className="absolute top-0 right-0 w-64 h-64 bg-brand-orange/15 rounded-full blur-[80px] pointer-events-none" />
          <div className="relative z-10 flex items-start space-x-4">
            ...
            <div>
              <h4 className="font-heading font-bold text-lg text-white mb-1">
                Authorized ISO Quality Standard Mark
              </h4>
              <p className="font-sans font-light text-sm text-gray-400 max-w-xl">
                All lamination products are manufactured under strict ISO 9001:2015 safety parameters, ensuring complete surface energy stability and compliance certifications.
              </p>
            </div>
          </div>
          <button
            onClick={() => onScrollTo('contact')}
            className="px-6 py-3 rounded-xl bg-brand-orange text-white text-xs font-bold uppercase tracking-wider hover:scale-105 active:scale-95 transition-all flex-shrink-0 cursor-pointer"
          >
